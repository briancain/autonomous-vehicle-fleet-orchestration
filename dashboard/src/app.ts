...
class FleetDashboard {
    ...
    private chargingStationMarkers: L.Marker[] = [];
    private jobMarkers: Map<string, L.Layer[]> = new Map(); // job_id -> [pickup, destination, line]
    ...
    private ensureChargingStations(): void {
        // Check if charging stations are still on the map
        const visibleStations = this.chargingStationMarkers.filter(marker => 
    ...
    private async updateDashboard(): Promise<void> {
        const [vehicles, jobs, revenue] = await Promise.all([
            this.fetchData<Vehicle[]>(`${API_BASE.fleet}/vehicles`),
            this.fetchData<Job[]>(`${API_BASE.jobs}/jobs`),
            this.fetchData<any>(`${API_BASE.jobs}/revenue`)
        ]);
    ...
    private updateMetrics(vehicles: Vehicle[], jobs: Job[]): void {
    ...
    private updateRevenue(revenue: any): void {
        if (revenue) {
    ...
    private updateMap(vehicles: Vehicle[]): void {
    ...
    private updateJobMarkers(jobs: Job[]): void {
    ...
    private updateJobList(jobs: Job[]): void {
    ...
    private showChargingVehicles(station: any): void {
        // Get current vehicles data
        fetch(`${API_BASE.fleet}/vehicles`)
            .then(response => response.json())
            .then(vehicles => {
                // Find vehicles that are charging near this station (within 100m)
                const chargingVehicles = vehicles.filter((vehicle: Vehicle) => {
                    if (vehicle.status !== 'charging') return false;
                ...
                const popupContent = `
                    ...
                    ${chargingVehicles.length > 0 
                        ? chargingVehicles.map((v: Vehicle) => 
                            `• ${v.id} (${v.battery_level}%)`
                          ).join('<br>')
    ...
    private updateVehicleList(vehicles: Vehicle[]): void {
    ...
    public focusOnVehicle(vehicleId: string): void {
    ...
    public focusOnJob(jobId: string): void {
